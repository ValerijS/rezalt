			    ...
			    if(curr_nod[0]) {
                    ...
                    if (curr_nod[0][1]) {
					    ...
					    if (randomElection([1, 0], [1 - (p_of_send_action * dist_of_pair[1]) * current_bucket_or_needle[3], (p_of_send_action * dist_of_pair[1]) * current_bucket_or_needle[3]])) {
							...
							curr_nod[0][5] = Math.max(curr_nod[0][5], t) + (use_time_for_performance_local_action_with_one_cond_unit_of_data * current_bucket_or_needle[4]) + dist_of_pair[0];//time for performance local action.
		...
		for (buck in gossip_protocol[2][key_of_bucket_or_needle]) {
            ...
            for (n = 0; n < cluster.length; n++) {
                        if (cluster[n][0] === buck) {
                            ...
                            cluster[n][5] = Math.max(cluster[n][5], t) +  use_time_for_performance_local_action_with_one_cond_unit_of_data * current_bucket_or_needle[4];//time for performance local action.
...
//input part:   
   ...
   alert(["total_count_of_data - " + Math.floor(total_count_of_data), // in all servers,
     ...
     console.log(gossip_protocol, 'con_gp_f',total_count_of_data / total_used_resource , (count_of_iterations_for_algorithm * level_of_intensivity_of_requests - (count_of_read_failures +  count_of_write_failures)) / (count_of_iterations_for_algorithm * level_of_intensivity_of_requests), ((count_of_iterations_for_algorithm * level_of_intensivity_of_requests) - count_of_read_lags) / (count_of_iterations_for_algorithm * level_of_intensivity_of_requests), 'write_new_actions = ' + w_ac, v, cluster, gateway_cluster, max_lag_of_performance_for_gateway_nodes, 'L = ' + L, 'M = ' + M, 'G =' + G, 'Int =' + Int,'manage-repair = ' + w);
