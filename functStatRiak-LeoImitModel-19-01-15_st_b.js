...
//Initial setting part:
    ...
    metaRiak_objects = [["text", 0.01, value_of_Riak_object, 0.1, 0.001], ["foto", 2, value_of_Riak_object, 1, 0.1], ["vidio", 50,  value_of_Riak_object, 3, 1]];//Value of Riak object = [type of Riak object, data size of Riak object in conditional units,  contxt(really - input time), coefficient  for probability of failure for this type, time size of Riak object in conditional units], type may be: ["text", 0.01, t, 0,1, 0.001], ["foto", 2, t, 1, 0.1], ["vidio", 50, t, 5, 1]. 
    ...
    switch (rem_type) {
        ...
        case "2":
            for (i=1; i <= count_of_nodes; i++) {
               matrix[i] = [];
               for (j = 1; j <= count_of_nodes; j++) {
                   matrix[i][j] = Math.floor((Math.min(Math.abs(i - j), count_of_nodes - Math.abs(i - j))/count_of_nodes) * 7) + 1;
               }
            }
            break;
        default:
...
// iteration's part, write action:
        ...
        if (current_action === write_action) {//then do a realisation of  the action.
            ...
            if (electOfEventWithProbability_p(probOfGetNewKeyInWriteReqwest + (1 - probOfGetNewKeyInWriteReqwest) / t)) {
				...
				gossip_protocol[2][key_of_bucket_or_needle]	= {}; 
			    ...
			    if(curr_nod[0]) {
                    ...
                    if (curr_nod[0][1]) {
					    ...
					    if (randomElection([1, 0], [1 - (p_of_send_action * dist_of_pair[1]) * current_bucket_or_needle[3], (p_of_send_action * dist_of_pair[1]) * current_bucket_or_needle[3]])) {
                            ...
                            gossip_protocol[9][key_of_bucket_or_needle][curr_nod[0][0]] = ['value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-and, then: -' , curr_nod[0][5], [t, u], 1];
							...
							c = c + 1;
                                ...
                                gossip_protocol[2][key_of_bucket_or_needle][curr_nod[0][0]] = ['beginning: no commit at ' + t +'_' + u +'( < w_value and local failue) value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-and, then: -' , curr_nod[0][5], [-t, u]];
								gossip_protocol[9][key_of_bucket_or_needle][curr_nod[0][0]] = ['value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '(local_failue) -and, then: -' , curr_nod[0][5], [t, u], 1];
                                
								gossip_protocol[10][key_of_bucket_or_needle][curr_nod[0][0]] = ['(loc) value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-and, then: -', curr_nod[0][5], [-t, u], '-pair: (< w_value and local failue) value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-and, then: -' ]; 
                                                                
								curr_nod[0][2][key_of_bucket_or_needle] = 0; 
								curr_nod[0][3] = curr_nod[0][3] + current_bucket_or_needle[1]; //fresh information.
                            ...
                            gossip_protocol[2][key_of_bucket_or_needle][curr_nod[0][0]] = ['beginning: no commit at '+ t +'_' + u +'(write_fall), value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-and, then: -' , curr_nod[0][5], [-t, u]];
                ...
                if (curr_nod[0][1]) {
					...
					if (randomElection([1, 0], [1 - (p_of_send_action * dist_of_pair[1]) * current_bucket_or_needle[3], (p_of_send_action * dist_of_pair[1]) * current_bucket_or_needle[3]])) {
                        ...
                        if (randomElection([1, 0], [1 - p_of_write_action * current_bucket_or_needle[3], p_of_write_action * current_bucket_or_needle[3]]))   { //write_action finished right.
                            ...
                            if (gossip_protocol[2][key_of_bucket_or_needle][nod][3]) {
                                    gossip_protocol[9][key_of_bucket_or_needle][nod][3] = 1;
                                    gossip_protocol[2][key_of_bucket_or_needle][nod][3] = 0;
                                }
							gossip_protocol[2][key_of_bucket_or_needle][nod] = [gossip_protocol[2][key_of_bucket_or_needle][nod][0] + ' no commit at ' + t +'_' + u +'-( < w_value), value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-and, then: -' , curr_nod[0][5], [-t, u], 0];
                            
                             gossip_protocol[10][key_of_bucket_or_needle][nod] = [7, curr_nod[0][5], [-t, u], gossip_protocol[2][key_of_bucket_or_needle][nod][0] + '-pair: add( < w_value), value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-,and -' ]; 
							...
							c = c + 1;
                                ...
                                gossip_protocol[9][key_of_bucket_or_needle][nod] = [ gossip_protocol[2][key_of_bucket_or_needle][nod][0] + t +'_' + u +'-' + current_bucket_or_needle[0] + ' (local failue)-and-', curr_nod[0][5], [t, u]];
                                if (gossip_protocol[2][key_of_bucket_or_needle][nod][3]) {
                                    gossip_protocol[9][key_of_bucket_or_needle][nod][3] = 1;
                                    gossip_protocol[2][key_of_bucket_or_needle][nod][3] = 0;
                                }
                                gossip_protocol[2][key_of_bucket_or_needle][nod] = [gossip_protocol[2][key_of_bucket_or_needle][nod][0] + ' no commit at ' + t +'_' + u +'-( < w_value,and local failue), value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-and, then: -' , curr_nod[0][5], [-t, u], 0];
								...
								gossip_protocol[10][key_of_bucket_or_needle][nod] = [ gossip_protocol[10][key_of_bucket_or_needle][nod] + t +'_' + u +'- (loc) -' + current_bucket_or_needle[0] + '-and-', curr_nod[0][5], [-t, u], gossip_protocol[2][key_of_bucket_or_needle][nod][0] + '-pair: add ( < w_value, and local failue)__' + t +'_' + u +'-' + current_bucket_or_needle[0] + '--and-'];
				    ...
				    gossip_protocol[10][key_of_bucket_or_needle][nod] = [gossip_protocol[10][key_of_bucket_or_needle][nod]+ 'status_fall--' + t +'_' + u +'-' + current_bucket_or_needle[0] + '--and-', curr_nod[0][5], [-t, u], gossip_protocol[2][key_of_bucket_or_needle][nod][0] + '-pair: (status_fall)-' + t +'_' + u +'-' + current_bucket_or_needle[0] + '--and-'];
...
console.log('reply to key ' + key_of_bucket_or_needle + ' is ',reply, '; read action ' + s2 + ' from ' + s1 + ' at step ' + t + '_' + u + '-_- at ' + time.toFixed(3) )
	...
	for (x in gossip_protocol[10]) {
					...
					cluster[m][5] = Math.max(cluster[m][5], t) + time_for_performance_repair_action;//time performance repair.
					total_time_of_finish_imitatiion = Math.max(total_time_of_finish_imitatiion, cluster[m][5]);
                    gossip_protocol[2][x][node] = ['_' + t + '--' + u + '_manager re- ' + gossip_protocol[10][x][node][3], cluster[m][5], [-gossip_protocol[10][x][node][2][0], gossip_protocol[10][x][node][2][1]],1];
